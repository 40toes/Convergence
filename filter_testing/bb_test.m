...
title('Cliff')
subplot(3,1,2)
cliff_f = 2*abs(fft(cliff_reshape));
plot(cliff_f)
title('Cliff FFT, lets not use this')
...
subplot(3,1,3)
plot(cliff_ed)
title('Make things more prominent in cliff spectrum')

%% Filtering
close all
% Try multiplying cliff in time domain with audio fft 
cliff_length = length(cliff_reshape);
% pad cliff with zeros so can multiply with audio spectrum
padded_cliff = ([cliff_reshape; zeros(length(bb_f)-cliff_length,1)])/10000; 
cliff_coeff_arr = [1000*ones(length(cliff_reshape)/5,1); 3000*ones(length(cliff_reshape)/5,1); 5000*ones(length(cliff_reshape)/5,1); 7000*ones(length(cliff_reshape)/5,1); 8000*ones(length(cliff_reshape)/5,1); zeros(length(bb_f)-cliff_length,1)]/1000;
padded_cliff = cliff_coeff_arr .* padded_cliff;
bb_filt_f = bb_f + padded_cliff.*bb_f;
bb_filt_re = abs(bb_filt_f);


padded_cliff_1 = padded_cliff(1:(length(cliff_reshape)/5));
padded_cliff_2 = padded_cliff((length(cliff_reshape)/5):(length(cliff_reshape)/5)*2);
padded_cliff_3 = padded_cliff((length(cliff_reshape)/5)*2:(length(cliff_reshape)/5)*2)*4;

bb_f_two_channel_filt_1 = [bb_f_two_channel((1:length(padded_cliff_1)),:)+bb_f_two_channel((1:length(padded_cliff_1)),:).*padded_cliff_1; bb_f_two_channel((1:length(padded_cliff_1)),)+bb_f_two_channel(:,(2:length(padded_cliff_1))).*padded_cliff_1];
bb_f_two_channel_filt_2 = [bb_f_two_channel(:,(length(padded_cliff_1):length(padded_cliff_1)*2))+bb_f_two_channel(:,(length(padded_cliff_1):length(padded_cliff_1)*2)).*padded_cliff_2; bb_f_two_channel(:,(length(padded_cliff_1):length(padded_cliff_1)*2))+bb_f_two_channel(:,(length(padded_cliff_1):length(padded_cliff_1)*2)).*padded_cliff_2;];

% do the same for bichannel signal
% "filtering" by multiplying both channels by padded cliff shape
...
subplot(4,1,1)
plot(cliff_reshape)
title('Cliff, original')
subplot(4,1,2)
plot(2*abs(bb_f))
axis([0 150000 0 12000])
title('Audio spectrum, original')
subplot(4,1,3)
plot(2*bb_filt_re(1:length(bb_filt_re)/2));
axis([0 150000 0 12000])
title('Audio spectrum x cliff shape')
subplot(4,1,4)
hold on
bb_filt = fft(bb_filt_f);
plot(2*abs(bb_filt));
plot(-2*abs(bb_filt));
hold off
title('both channels')

bb_two_channel_filt_cliff = ifft(bb_f_two_channel_filt); % result of cliff shape filter

% Highlight certain freqencies with just a band. Try shifting band filter by 500
front_pad = 50;
band_length = 5000;
band_coeff = 10;
band = [ zeros(front_pad,1); band_coeff*ones(band_length,1); zeros(length(bb_f)-band_length-front_pad,1)];
bb_f_two_channel_filt = [bb_f_two_channel(:,1)+bb_f_two_channel(:,1).*band; bb_f_two_channel(:,2)+bb_f_two_channel(:,2).*band]; % result of band filter
...
subplot(4,1,3)
plot(2*abs(bb_f_two_channel_filt))
axis([0 150000 0 12000])
title('Resultant audio spectrum')
subplot(4,1,4)
hold on
plot(2*abs(fft(bb_f_two_channel_filt)))
plot(-2*abs(fft(bb_f_two_channel_filt)))
title('Resultant audio')
hold off
...
% Play back 
player = audioplayer(bb_two_channel_filt_cliff,Fs*2);
play(player)